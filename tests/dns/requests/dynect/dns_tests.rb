Shindo.tests('Dynect::dns | DNS requests', ['dynect', 'dns']) do
  ...
  tests "success" do
    ...
    tests("post_record('A', '#{@domain}', 'www.#{@domain}', 'address' => '1.2.3.4')").formats(post_record_format) do
      Dynect[:dns].post_record('A', @domain, "www.#{@domain}", 'address' => '1.2.3.4').body
    end
    ...
    tests("put_zone('#{@domain}', :publish => true)").formats(put_zone_format) do
      Dynect[:dns].put_zone(@domain, :publish => true).body
    end

    @record_id = Dynect[:dns].get_record('ANY', @domain, "www.#{@domain}").body['data'].first.split('/').last
